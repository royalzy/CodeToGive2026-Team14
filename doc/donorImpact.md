...
remains non-persistent: it does not process payments, save personal information,
send email, or connect to a CRM.
...
The competition version does not accept credit card or payment information. The existing backend explicitly defines the donation intent as a simulated flow with no database, payment processing, authentication, email, or CRM integration.
